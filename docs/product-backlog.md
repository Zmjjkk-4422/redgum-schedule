...
Epic 1 **Redgum scheduling core (Sprint 1)** contains the 15 committed
stories (49 story points). Epic 2 **Parked product backlog** records every
...
| RED-02 | As a developer, I want persistent students, tutors, windows and sessions in SQLite so data survives restarts | Schema + repository CRUD; FK integrity; CHECK constraints; repository tests | 3 | Han |
| RED-03 | As Deb, I want bookings checked against tutor availability so sessions cannot be booked when the tutor is unavailable | Rule service; boundary/gap/no-window/deactivated cases; negative tests; plain-language rejection reasons | 3 | Han |
...
| RED-13 | As Helen, I want bookings refused past a tutor's weekly cap so tutors such as Tomas (max 8) are never overbooked | Booked/attended count toward cap; cap+1 refused with reason; boundary and move cases; negative tests | 3 | Max |
| RED-14 | As Deb, I want overlaps refused for the same tutor or student so nobody is double-booked | Overlap on create/move refused with conflict named; touching slots allowed; cancelled/missed ignored; tests | 3 | Max |
...
| RED-15 | As Deb, I want a warning when removing a window strands existing bookings so mid-term amendments never silently break the schedule | Window removal checks remaining windows; affected booked sessions listed, never deleted; all-clear state; tests | 2 | Jiao |

**Total committed: 49 points (Han 20, Max 14, Jiao 15).**
...
| PARK-02 | Room-based clash checks across tutors (rooms, gaps, Room 3 capacity) | Helen/Deb | Rooms excluded from the fixed sprint scope; same-tutor/student overlap is delivered as RED-14; room clashes revisit with the room model |
